...
import ComparableSearch from '@bundles/advanced-search/components/comparable-search';
import GeneralSearch from '@bundles/advanced-search/components/general-search';
import SubmitGroup from '@bundles/advanced-search/components/submit-group';
import TextSearch from '@bundles/advanced-search/components/text-search';
import bindAll from '@lib/helpers/bind-all';
import { pipe } from '@lib/helpers/pipe';

export default class AdvancedSearch extends React.Component {
  static propTypes = {
    filters: PropTypes.objectOf(
      PropTypes.shape({
        type: PropTypes.oneOf(['general-search', 'text', 'number', 'date', 'datetime-local'])
          .isRequired,
  ...
  _initDefaultFilters(state) {
    ...
    const firstValues = Object.fromEntries(
      searchQuery
        .reverse()
        .map(({ condition, field, type, value }) => [`${field}-${type}`, { value, condition }])
    );

    Object.entries(config).every(([field, { type, isRenderOnStart }], index) => {
      if (isRenderOnStart) {
  ...
  _newFilterOptions() {
    const searchState = this.state.search;
    const configs = this.props.filters;
    const existingFilters = Object.values(searchState).reduce(
